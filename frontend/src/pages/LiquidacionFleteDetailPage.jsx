    ...
    <div className="space-y-6 page-fade-in" data-testid="liquidacion-detail-page">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        ...
        <Card className="bg-white border-l-4 border-l-purple-500"><CardContent className="py-4">
          <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Detracción</p>
          <p className="font-heading text-xl font-bold text-purple-600 mt-1">{soles(liquidacion.total_detraccion)}</p>
        </CardContent></Card>
        <Card className="bg-white border-l-4 border-l-slate-400"><CardContent className="py-4">
          <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Viáticos</p>
          <p className="font-heading text-xl font-bold text-slate-700 mt-1">{soles(liquidacion.total_viaticos)}</p>
        </CardContent></Card>
                ...
                lineas.map((l) => (
                  <TableRow key={l.id} className="table-dense" data-testid={`linea-row-${l.id}`}>
                    ...
                    <TableCell>
                      ...
                      <span className="block text-xs text-slate-400">{l.conductor_nombre || '-'}</span>
                    </TableCell>
                      ...
                      <div className="flex gap-1">
                        {DOC_SLOTS.map(({ kind, field, label }) => (
                          <span
                            key={kind}
                            title={label}
                            className={`w-2 h-2 rounded-full ${l[field] ? 'bg-green-500' : 'bg-slate-200'}`}
                          />
                        ))}
                      </div>
                    </TableCell>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {DOC_SLOTS.map(({ kind, label }) => (
                  <div key={kind} className="border rounded-lg p-3 space-y-2 bg-slate-50">
                    <p className="text-xs font-semibold text-slate-600">{label}</p>
                    {docPreview[kind] ? (
                      <img src={docPreview[kind]} alt={label} className="w-full h-24 object-cover rounded" />
                    ) : (
                      <div className="w-full h-24 flex items-center justify-center bg-white border border-dashed rounded text-slate-300">
                    ...
                    {docStatus[kind] === 'uploading' && <p className="text-xs text-slate-400 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Guardando...</p>}
                  </div>
                ))}
              </div>
            </div>
