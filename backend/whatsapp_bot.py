...
async def _process_message(message: dict) -> None:
    ...
    await srv.db.whatsapp_events.insert_one({
        "id": str(uuid.uuid4()),
        "wa_id": wa_id,
        "msg_type": msg_type,
    ...
    if not driver:
        await srv.db.whatsapp_unrecognized.insert_one({
            "id": str(uuid.uuid4()),
            "wa_id": wa_id,
            "created_at": datetime.now(timezone.utc).isoformat(),
        })
    ...
    if msg_type == "text":
        pending = await srv.db.whatsapp_pending_selection.find_one(
            {"wa_id": wa_id, "expires_at": {"$gt": datetime.now(timezone.utc).isoformat()}}
        )
        if pending:
    ...
    if len(trips) > 1:
        ...
        await srv.db.whatsapp_pending_selection.insert_one({
            "id": str(uuid.uuid4()),
            "wa_id": wa_id,
            ...
            "expires_at": (datetime.now(timezone.utc) + timedelta(minutes=PENDING_SELECTION_TTL_MINUTES)).isoformat(),
            "created_at": datetime.now(timezone.utc).isoformat(),
        })
...
async def _resolve_pending_selection(pending: dict, reply_text: str) -> None:
    ...
    await srv.db.whatsapp_pending_selection.delete_one({"id": pending["id"]})
...
async def _ingest_document(company_id: str, driver_id: str, trip_id: str, wa_id: str, media_id: str) -> None:
    ...
    await srv.db.whatsapp_documentos_pendientes.insert_one({
        "id": pendiente_id,
        "company_id": company_id,
        "trip_id": trip_id,
        "driver_id": driver_id,
...
async def get_whatsapp_pendientes(
    ...
    pendientes = await srv.db.whatsapp_documentos_pendientes.find(
        {"company_id": current_user["company_id"], "status": status}, {"_id": 0}
    ).sort("created_at", -1).to_list(500)
    return [srv.serialize_doc(p) for p in pendientes]
...
async def asignar_whatsapp_pendiente(
    ...
    pendiente = await srv.db.whatsapp_documentos_pendientes.find_one(
        {"id": pendiente_id, "company_id": company_id}, {"_id": 0}
    )
    if not pendiente:
    ...
    kind = pendiente.get("detected_kind")
    field = DOCUMENTO_KINDS.get(kind)
    if not field:
        raise HTTPException(status_code=400, detail="Tipo de documento no reconocido, revisa manualmente")

    # liquidacion_lineas ya cortó a Postgres (db/migrations/001_corte_liquidacion_flete.sql).
    # Este endpoint la escribe igual que liquidacion_flete.py, así que tiene que
    # hablarle a Postgres: si siguiera escribiendo en Mongo, esa tabla tendría dos
    # fuentes de verdad y los datos divergirían en silencio.
    # whatsapp_documentos_pendientes, en cambio, sigue en Mongo (aún no cortó).
    ...
    if not linea_id:
        ...
        async with db_pg.tx(current_user) as conn:
            ...
            await conn.execute(sql, *values)
    await srv.db.whatsapp_documentos_pendientes.update_one(
        {"id": pendiente_id},
        {"$set": {"status": "asignado", "linea_id": linea_id, "assigned_by": current_user["id"],
